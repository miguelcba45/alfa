window.angularApp.controller("PosController", [
    ...
    function (
        ...
        $scope._calcOthersCharge = function () {
            if (window._.includes($scope.othersChargeInput, '%')) {
                $scope.othersChargeType = 'percentage';
            } else {
                $scope.othersChargeType = 'plain';
            }
        ...
        $scope._calcTotalPayable = function ($childScope) {
            ...
            if ($scope.shippingType == 'percentage') {
                shippingPercentage = parseFloat($scope._percentage($scope.payable, $scope.shippingAmount));
            } else {
                shippingPercentage = parseFloat($scope.shippingAmount);
            }

            if ($scope.othersChargeType == 'percentage') {
                othersPercentage = parseFloat($scope._percentage($scope.payable, $scope.othersCharge));
            } else {
                othersPercentage = parseFloat($scope.othersCharge);
            }

            $scope.payable = ($scope.payable + shippingPercentage + $scope.dueAmount) - discountPercentage - ($scope.payable * (othersPercentage / 100));

            // Ajustar el total para que no tenga decimales y redondear hacia arriba si es necesario
            $scope.totalPayable = Math.round($scope.payable + $scope.installmentInterestAmount);
